...
object Guidata {
	...
	if (pricecompareconfig.hasPath("scroll")){
		if (pricecompareconfig.getString("scroll").toLowerCase.contains("yes")){
			scroll = true
		}
	}
...
object Pricecompare extends JFXApp3 {
	...
	val help_online_url = "http://www.jvr.de/" + appname + "/README.html"
...
//----------------------------------- start -----------------------------------
	...
	def readUrlFile(urlString: String) = {
		...
		var ll = 0
		try {
			val lines = urlString.toFile.lineIterator.toList
			ll = lines.length
	...
	def compare(urlFileName: String) = {
		...
		t._1 match {
			case Right(x) => {
				for (i <- 0 until Guidata.rowsMAX) rowsBuffer += new PricecompareResultRow((i + 1).toString, "", "", "", Color.Red)
